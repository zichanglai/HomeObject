...
TEST_F(HomeObjectFixture, BasicPutGetBlob) {
    ...
    uint32_t max_blob_size = 1024;
    uint32_t alignment = 1024;
    for (uint64_t i = 1; i <= num_pgs; i++) {
        create_pg(i /* pg_id */);
        for (uint64_t j = 0; j < num_shards_per_pg; j++) {
            auto shard = _obj_inst->shard_manager()->create_shard(i /* pg_id */, 10 * Mi).get();
            ASSERT_TRUE(!!shard);
    ...
    for (const auto& id : pg_shard_id_vec) {
        ...
        for (uint64_t k = 0; k < num_blobs_per_shard; k++) {
            std::string user_key;
            user_key.reserve(32);
            BitsGenerator::gen_random_bits(32, (uint8_t*)user_key.data());
            homeobject::Blob put_blob{sisl::io_blob_safe(max_blob_size, alignment), user_key, 0ul};
            BitsGenerator::gen_random_bits(put_blob.body);
            // Keep a copy of random payload to verify later.
            homeobject::Blob clone{sisl::io_blob_safe(max_blob_size, alignment), user_key, 0ul};
    ...
    for (const auto& [id, blob] : blob_map) {
        ...
        LOGINFO("Get blob pg {} shard id {} blob id {} data {}", pg_id, shard_id, blob_id,
                hex_bytes(result.body.bytes, 10));
    ...
    for (const auto& [id, blob] : blob_map) {
        ...
        LOGINFO("After restart get blob pg {} shard {} blob {} data {}", pg_id, shard_id, blob_id,
                hex_bytes(result.body.bytes, 10u));
    ...
    std::uniform_int_distribution< uint32_t > rand_off_gen{0u, max_blob_size - 1u};
    std::uniform_int_distribution< uint32_t > rand_len_gen{1u, max_blob_size};
