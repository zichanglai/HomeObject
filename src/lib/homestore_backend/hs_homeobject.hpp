...
#pragma pack(1)
    // Every blob stored in disk as blob header | blob data | blob metadata(optional).
    struct BlobHeader {
        ...
        shard_id_t shard_id;
        int64_t meta_data_offset{}; // Metadata stored after the blob data.
        int64_t total_size{};       // Size of data and metadata.

        bool valid() const { return magic == blob_header_magic || version <= blob_header_version; }
        std::string to_string() {
            return fmt::format("magic={} version={} algo={} hash={} shard={} meta_offset={} total_size={}", magic,
                               version, (uint8_t)hash_algorithm, hex_bytes(hash, blob_max_hash_len), shard_id, meta_data_offset,
                               total_size);
        }
        bool compute_blob_hash(const sisl::blob& blob, bool verify);
    };
#pragma pack()
